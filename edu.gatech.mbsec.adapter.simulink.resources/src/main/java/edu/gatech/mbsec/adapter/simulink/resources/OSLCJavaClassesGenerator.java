...
public class OSLCJavaClassesGenerator {
	...
	static String simulinkEcoreGitRepositoryLocation = "C:/Users/Axel/git/org.eclipse.lyo.adapter.simulink3/";		// necessary to know the entire file path to resolve URIs		
	...
	static String javaSrcPackageLocation = "C:/Users/Axel/git/org.eclipse.lyo.adapter.simulink3/edu.gatech.mbsec.adapter.simulink.resources/src/main/java/org/eclipse/lyo/adapter/simulink/resources/";
	
	static String simulinkEcoreLocation = simulinkEcoreGitRepositoryLocation + "edu.gatech.mbsec.adapter.simulink.ecore/model/simulink4MBSE.ecore";	// assuming that the .ecore metamodel is named simulink4MBSE.ecore
	...
	public static void main(String[] args) {
		...
		Resource ecoreResource = loadEcoreModel(URI.createFileURI(new File(simulinkEcoreLocation).getAbsolutePath()));
		simulinkPackage = (EPackage) EcoreUtil.getObjectByType(ecoreResource.getContents(),
		...
		for (EClassifier eclassifier : simulinkPackage.getEClassifiers()) {
			if (eclassifier instanceof EClass) {
				...
				if (!eclass.isAbstract()) {
					...
					try {
						fileWriter = new FileWriter(javaSrcPackageLocation + "Simulink" + eclass.getName() + ".java");
		...
		if(superTypesCount > 0){
			...
			for (EClass superClass : eclass.getEAllSuperTypes()) {
				if(superClass.eIsProxy()){
					...
					String resolvedURIString = unresolvedURIString.replace(simulinkEcoreGitRepositoryLocation, commonMBSEEcoreGitRepositoryLocation);
	...
	private static void printJavaClassReferences(StringBuffer buffer) {
		for (EReference ereference : eclass.getEAllReferences()) {
			...
			// find name of class containing reference
			String referenceContainingClassName = ereference.getEContainingClass().getName();
			...
			if (lowerBound >= 0 & upperBound == -1 | lowerBound >= 0 & upperBound >= 2) {
				...
				if (referenceName.endsWith("y")) {
					referenceName = referenceName.substring(0, referenceName.length() - 1) + "ie";
					referenceNameUpperCase = referenceNameUpperCase.substring(0, referenceNameUpperCase.length() - 1)
							+ "ie";
				...
				// print reference declaration
				buffer.append("\tprivate final Set<Link> " + referenceName + "s = new HashSet<Link>();");
				buffer.append("\r\n");
				buffer.append("\r\n");

				// print reference setter method
				buffer.append("\tpublic void set" + referenceNameUpperCase + "s(final Link[] " + referenceName + "s) {");
				buffer.append("\r\n");
				...
				buffer.append("\t\t{");
				buffer.append("\r\n");
				buffer.append("\t\t\tthis." + referenceName + "s.addAll(Arrays.asList(" + referenceName + "s));");
				buffer.append("\r\n");
				buffer.append("\t\t}");
				buffer.append("\r\n");
				buffer.append("\t}");
				...
				// @OslcDescription
				buffer.append("\t@OslcDescription(\"Description of " + referenceContainingClassName + "::"
				...
				// @OslcPropertyDefinition
				buffer.append("\t@OslcPropertyDefinition(\"" + Constants.SIMULINK_NAMESPACE
						+ referenceContainingClassName + "_" + originalReferenceName + "\")");
				buffer.append("\r\n");
				...
				// print reference getter method
				buffer.append("\tpublic Link[] " + " get" + referenceNameUpperCase + "s() {");
				buffer.append("\r\n");
				buffer.append("\t\t return " + referenceName + "s.toArray(new Link[" + referenceName + "s.size()]);");
				buffer.append("\r\n");
				buffer.append("\t}");
				buffer.append("\r\n");
				buffer.append("\r\n");
				...
				// print reference setter method
				buffer.append("\tpublic void set" + referenceNameUpperCase + "(final URI " + referenceName + ") {");
				buffer.append("\r\n");
				buffer.append("\t\tthis." + referenceName + " = " + referenceName + ";");
				buffer.append("\r\n");
				buffer.append("\t}");
				buffer.append("\r\n");
				buffer.append("\r\n");
				...
				// + etype.getName() + "\")");
				buffer.append("\t@OslcRange(\"" + Constants.SIMULINK_NAMESPACE + etype.getName() + "\")");
				buffer.append("\r\n");
	...
	private static void printJavaClassAttributes(StringBuffer buffer) {
		for (EAttribute eattribute : eclass.getEAllAttributes()) {
			...
			String attributeNameFirstLetter = attributeName.subSequence(0, 1).toString().toUpperCase();
			String attributeNameRest = attributeName.subSequence(1, attributeName.length()).toString();
			String attributeNameUpperCase = attributeNameFirstLetter + attributeNameRest;

			// find name of class containing attribute
			String attributeContainingClassName = eattribute.getEContainingClass().getName();
			...
			if (lowerBound >= 0 & upperBound <= 1) {
				...
				buffer.append("\tprivate " + attributeTypeName + " " + attributeName + ";");
				buffer.append("\r\n");
				buffer.append("\r\n");

				// print attribute setter method
				buffer.append("\tpublic void set" + attributeNameUpperCase + "(" + attributeTypeName + " "
						+ attributeName + ") {");
				buffer.append("\r\n");
				buffer.append("\t\tthis." + attributeName + " = " + attributeName + ";");
				buffer.append("\r\n");
				buffer.append("\t}");
				buffer.append("\r\n");
				buffer.append("\r\n");
				...
				// @OslcDescription
				buffer.append("\t@OslcDescription(\"Description of " + attributeContainingClassName + "::"
						+ attributeName + " TBD\")");
				buffer.append("\r\n");
				...
				// @OslcPropertyDefinition
				buffer.append("\t@OslcPropertyDefinition(\"" + Constants.SIMULINK_NAMESPACE
						+ attributeContainingClassName + "_" + attributeName + "\")");
				buffer.append("\r\n");
				...
				buffer.append("\tpublic " + attributeTypeName + " get" + attributeNameUpperCase + "() {");
				buffer.append("\r\n");
	...
	private static void printJavaClassConstructors(StringBuffer buffer) {
		buffer.append("\tpublic Simulink" + eclass.getName() + "() throws URISyntaxException {");
		buffer.append("\r\n");
		buffer.append("\t\tsuper();");
		buffer.append("\r\n");
		buffer.append("\t}");
		...
		buffer.append("\tpublic Simulink" + eclass.getName() + "(URI about) throws URISyntaxException {");
		buffer.append("\r\n");
	...
	private static void printJavaClassDeclaration(StringBuffer buffer) {
		if (eclass.getName().equals("Requirement")) {
			buffer.append("public class Simulink" + eclass.getName() + " extends Requirement{");
			buffer.append("\r\n");
			buffer.append("\r\n");
		} else {
			buffer.append("public class Simulink" + eclass.getName() + " extends AbstractResource{");
			buffer.append("\r\n");
			buffer.append("\r\n");
		}
	...
	private static void printJavaEnum(StringBuffer buffer, EEnum eEnum) {
		...
		for (Iterator iterator = eEnum.getELiterals().iterator(); iterator.hasNext();) {
	...
	private static void printJavaClassOSLCAnnotations(StringBuffer buffer) {
		...
		buffer.append("@OslcResourceShape(title = \"" + eclass.getName() + " Resource Shape\", describes = "
				+ "Constants.TYPE_SIMULINK_" + eclass.getName().toUpperCase() + ")");
		buffer.append("\r\n");
	...
	private static void printPackageDeclaration(StringBuffer buffer) {
		buffer.append("package edu.gatech.mbsec.adapter.simulink.resources;");
		buffer.append("\r\n");
		buffer.append("\r\n");
	}
...
//		ResourceSet resourceSet = new ResourceSetImpl();
	...
	public static EClass getEClass(String eClassName) {
		if (simulinkPackage == null) {
			// load SIMULINK.ecore model
			Resource ecoreResource = loadEcoreModel(URI.createFileURI(new File("simulink.ecore").getAbsolutePath()));
