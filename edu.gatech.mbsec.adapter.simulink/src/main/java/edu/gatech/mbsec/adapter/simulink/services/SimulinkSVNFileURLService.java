 ...
 * Simulink files to be published by the Simulink adapter basedon on Subversion
 * file URLs.
 * 
...
public class SimulinkSVNFileURLService {
	...
	public void get() {
		...
		CSVReader reader2;
		try {
			reader2 = new CSVReader(new FileReader(OSLC4JSimulinkApplication.svnURLsFilePath));
			List<String[]> allElements = reader2.readAll();
			if (svnurls == null) {
				if (allElements.size() > 0) {
					svnurls = OSLC4JSimulinkApplication.readSVNFileURLs(allElements);
				}
			}
		...
		httpServletRequest.setAttribute("creatorUri", SimulinkManager.baseHTTPURI + "/services/svnfilepublisher");
	...
	public Response add(@FormParam("svnurl") final String svnurl) throws IOException, ServletException {
		...
		try {
			...
			if (!svnurls.contains(svnurl)) {
				...
				// write to file
				CSVWriter writer2 = new CSVWriter(new FileWriter(OSLC4JSimulinkApplication.svnURLsFilePath), '\n');
	...
	public Response delete(@FormParam("svnurl") final String svnurl) throws IOException, ServletException {
		...
		new File(OSLC4JSimulinkApplication.simulinkModelsDirectory + localSubversionFileDir + "/" + subversionFileName)
				.delete();
	...
	public Response deleteAll() throws IOException, ServletException {
		...
		for (String subversionFileURLString : svnurls) {
			...
			String localSubversionFileDir = subversionFileDirURL.replace(":", "");
			localSubversionFileDir = localSubversionFileDir.replace("/", "");
			new File(OSLC4JSimulinkApplication.simulinkModelsDirectory + localSubversionFileDir + "/"
...
//					OSLC4JSimulinkApplication.simulinkModelsDirectory + localSubversionFileDir);
//		}
//		
//		
