...
/**
 * This is the OSLC service provider catalog for the Simulink adapter.  Service providers are
 ...
 * method.  A list of accessible Simulink models is retrieved and a ServiceProvider is
 * created and registered for each using the Simulink model name as the identifier.
 * 
...
public class ServiceProviderCatalogSingleton
{
    ...
    static
    {
        try
        {
            ...
            serviceProviderCatalog.setPublisher(new Publisher("Georgia Institute of Technology", "edu.gatech.mbsec.research.oslc"));
 ...
 */
    ...
    /**
     * Retrieve a list of Simulink models and construct a service provider for each.
     * 
     * Each project ID is added to the parameter map which will be used during service provider
     * creation to create unique URI paths for each Simulink model.  
     * 
		...
		try {
			...
			for (Model simulinkModel : SimulinkManager.simulinkWorkingDirectory.getModel()) {
				if (! serviceProviders.containsKey( simulinkModel.getName())) {
					...
					// parameter map captures parameter names used in JAX-RS @Path annotations
					parameterMap.put("modelName", simulinkModel.getName());
					final ServiceProvider simulinkServiceProvider = SimulinkServiceProviderFactory.createServiceProvider(basePath, simulinkModel.getName(), parameterMap);
